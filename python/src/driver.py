...
logging.basicConfig(filename='driver.log', level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
...
def score_data(a):
    logger = logging.getLogger('score_data')  # Create a logger for this function
    hdao = dao.HBaseDao.get_instance()
    try:
        data_fetch = hdao.get_data(key=a, table='lookup_data_hive')
        logger.info(f'Retrieved data for key {a}: {data_fetch}')
        return data_fetch['info:score']
    except Exception as e:
        logger.error(f'Error in score_data for key {a}: {str(e)}')
...
parsed_df = parsed_df.withColumn('last_transaction_date_ts',
                                 unix_timestamp(parsed_df.last_transaction_date, 'YYYY-MM-dd HH:mm:ss').cast(TimestampType()))
...
def status_find(card_id, member_id, amount, pos_id, postcode, transaction_dt, transaction_dt_ts, last_transaction_date_ts, score, speed):
    ...
    look_up = hdao.get_data(key=card_id, table='lookup_data_hive')
    status = 'FRAUD'
    if rules.rules_check(look_up['info:UCL'], score, speed, amount):
        status = 'GENUINE'
        look_up['info:transaction_date'] = str(transaction_dt_ts)
        look_up['info:postcode'] = str(postcode)
        hdao.write_data(card_id, look_up, 'lookup_data_hive')
